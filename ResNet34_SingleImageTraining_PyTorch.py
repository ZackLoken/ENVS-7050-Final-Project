...
model = models.resnet34(pretrained=True) # Don't use pretrained for your actual master's work. 
# print(model)
...
model.fc = nn.Sequential(nn.Linear(512, 128), # Re-define the final fully-connected layer that will be
                                 nn.ReLU(),    # trained with the images in the training dataset. 
                                 nn.Dropout(0.2), # if overfitting (training loss < validation loss) by a lot, try increasing your dropout value (e.g., 0.5)
                                 nn.Linear(128, len(trainloader.dataset.classes)),
                                 nn.LogSoftmax(dim=1))
criterion = nn.NLLLoss() # create the loss function (criterion)
...
for epoch in range(epochs):
    for inputs, labels in trainloader:
        ...
        if steps % print_every == 0:
            ...
            running_loss = 0
            model.train()
torch.save(model, 'ResNet34_PyTorch.pth')
...
# plt.plot(train_losses, label='Training loss')
# plt.plot(test_losses, label='Validation loss')
# plt.title("ResNet 34 - Validation Loss")
# plt.xlabel("# of Epochal Steps (5 epochs/step)")
# plt.ylabel("Loss")
# plt.legend(frameon=False,
#             loc = 'best')
